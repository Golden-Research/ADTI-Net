...
def get_args_parser():
    ...
    parser.add_argument('--num_s_dtd_layers', default=2, type=int,
                        help="Number of spatially-decoupled decoder layers "
                             "in each ADTD branch (paper: 2)")
    parser.add_argument('--num_t_dtd_layers', default=3, type=int,
                        help="Number of temporally-decoupled decoder layers "
                             "in each ADTD branch (paper: 3)")
    parser.add_argument('--attn_window', default=256, type=int,
                        help="Local window size G of the spatial/temporal "
                             "mask self-attention (paper: 256)")
    parser.add_argument('--max_seq_frames', default=30, type=int,
                        help="Maximum number of frames per sequence, i.e. "
                             "the number of levels of the temporal "
                             "deformable attention (paper: N=30)")
    parser.add_argument('--film_tau', default=0.5, type=float,
                        help="Temperature tau of the imitation loss (paper: 0.5)")
    parser.add_argument('--film_dim', default=128, type=int,
                        help="Dimension of the textual features / imitation "
                             "embedding space (paper: 128)")
    parser.add_argument('--film_loss_coef', default=1.0, type=float,
                        help="Weight of the imitation (TFIL) loss")
    parser.add_argument('--text_model',
                        default='openai/clip-vit-base-patch32', type=str,
                        help="CLIP text encoder used by TFIL")
    parser.add_argument('--num_ref_frames_eval', default=None, type=int,
                        help="Number of reference frames during evaluation "
                             "(defaults to --num_ref_frames). ADTI-Net uses "
...
def main(args):
    ...
    # ADTI-Net: the TFIL modules (textual projection, quality indicator,
    # imitation embedding) live inside the criterion and must be optimized
    if args.model_type == 'adti':
        criterion_params = [p for p in criterion.parameters() if p.requires_grad]
        if criterion_params:
            param_dicts[0]["params"] = param_dicts[0]["params"] + criterion_params
    ...
    if args.distributed:
        ...
        # ADTI-Net: wrap the criterion too, so that the gradients of the
        # TFIL modules are synchronized across GPUs
        if any(p.requires_grad for p in criterion.parameters()):
            criterion = torch.nn.parallel.DistributedDataParallel(
                criterion, device_ids=[args.gpu], find_unused_parameters=True)
    ...
    if args.resume:
        ...
        if args.eval:
            ...
            if args.model_type == 'adti' and 'criterion' in checkpoint:
                criterion_wd = criterion.module if hasattr(criterion, 'module') else criterion
                criterion_wd.load_state_dict(checkpoint['criterion'])
        else:
            if args.model_type == 'adti':
                # ADTI-Net is trained end-to-end; no parameter freezing.
                # Also restore the TFIL modules when resuming.
                tmp_dict = checkpoint['model']
                if 'criterion' in checkpoint:
                    criterion_wd = criterion.module if hasattr(criterion, 'module') else criterion
                    criterion_wd.load_state_dict(checkpoint['criterion'])
            elif args.coco_pretrain: # singleBaseline
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            for checkpoint_path in checkpoint_paths:
                ...
                if args.model_type == 'adti':
                    criterion_wd = criterion.module if hasattr(criterion, 'module') else criterion
                    ckpt['criterion'] = criterion_wd.state_dict()
