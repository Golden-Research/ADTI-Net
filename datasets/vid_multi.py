...
class CocoDetection(TvCocoDetection):
    def __init__(self, img_folder, ann_file, transforms, return_masks, interval1, interval2, num_ref_frames= 3,
        ...
        # ADTI-Net: sample the reference frames uniformly over the whole
        # video at evaluation time (sequence-wise detection with N frames)
        self.eval_full_seq = eval_full_seq
    ...
    def __getitem__(self, idx):
        ...
        if video_id == -1: # imgnet_det
            ...
            if self.is_train: # Train
                ...
                ref_img_ids = []
                Len = len(img_ids)
                if self.eval_full_seq:
                    # ADTI-Net: uniform sampling of the whole video so that
                    # the N-frame input covers the entire sequence
                    interval = max(int(Len // self.num_ref_frames), 1)
                else:
...
def build(image_set, args):
    ...
    for (img_folder, ann_file) in PATHS[image_set]:
        # ADTI-Net: allow a different number of reference frames at
        # evaluation (paper: N=30 frames at inference, 1+4 during training)
        num_ref = args.num_ref_frames
        if image_set == 'val' and getattr(args, 'num_ref_frames_eval', None):
            num_ref = args.num_ref_frames_eval
        dataset = CocoDetection(img_folder, ann_file, transforms=make_coco_transforms(image_set), is_train =(not args.eval), interval1=args.interval1,
                                interval2=args.interval2, num_ref_frames = num_ref, return_masks=args.masks, cache_mode=args.cache_mode, 
                                local_rank=get_local_rank(), local_size=get_local_size(),
                                eval_full_seq=(getattr(args, 'model_type', 'transvod') == 'adti'))
